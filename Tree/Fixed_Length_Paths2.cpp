...
int get_subtree_sizes(int node, int parent = 0) {
	subtree[node] = 1;
	for (int i : adj[node])
		if (!dead[i] && i != parent) subtree[node] += get_subtree_sizes(i, node);
	return subtree[node];
}
 
int get_centroid(int desired, int node, int parent = 0) {
	for (int i : adj[node])
		if (!dead[i] && i != parent && subtree[i] >= desired)
...
void get_cnt(int node, int parent, int depth = 1) {
	...
	cnt[depth]++;
	for (int i : adj[node])
		if (!dead[i] && i != parent) get_cnt(i, node, depth + 1);
}
